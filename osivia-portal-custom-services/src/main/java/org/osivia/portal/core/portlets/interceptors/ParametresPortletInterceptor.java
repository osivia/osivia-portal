			...
			if( windowId.charAt(0) == CanonicalFormat.PATH_SEPARATOR)	{
				...
				if (menuBar != null) {
					...
					if ("1".equals(printPortlet)) {
						...
						menuBar.add(new MenubarItem("Imprimer", 100, "#", "popup2print('" + jsTitle + "', '" + windowId
								+ "_print');", "portlet-menuitem-print", null));
					}
					...
					if (menuBar.size() > 0) {
						...
						return new FragmentResponse(fr.getProperties(), fr.getAttributes(), fr.getContentType(),
								fr.getBytes(), updatedFragment, fr.getTitle(), fr.getCacheControl(), fr.getNextModes());
					}
				}
