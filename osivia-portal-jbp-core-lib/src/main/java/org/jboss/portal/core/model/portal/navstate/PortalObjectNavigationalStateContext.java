...
/*
 * OSIVIA
 * Add support for real navigation in browser
 */
...
public class PortalObjectNavigationalStateContext implements NavigationalStateContext {
    ...
    public PortalObjectNavigationalStateContext(ControllerContext ctx, AttributeResolver store) {
        ...
        //logger.info("create " + getViewId() + " request " + request.toString());
    }
    ...
    public Object getAttribute(Object attrKey) throws IllegalArgumentException {
        ...
        return store.getAttribute(storeKey);
    }
    ...
    public void setAttribute(Object attrKey, Object newNS) throws IllegalArgumentException {
        if (attrKey instanceof NavigationalStateKey) {
            ...
            /*
            if (newNS != null && newNS instanceof WindowNavigationalState) {

                WindowNavigationalState windowState = (WindowNavigationalState) newNS;


                if (wantedKey.getId().toString().endsWith("CMSPlayerWindow")) {

                    logger.info("set CHANGE CMSPlayerWindow " + ((WindowNavigationalState) newNS).getWindowState());
        ...
        if (attrKey instanceof String) {
            store.setAttribute(attrKey, newNS);
        }
    }
    ...
    public boolean applyChanges() {
        ...
        // logger.info("applyChanges v=" + getViewId());
            ...
            for (NavigationalStateObjectChange change : changes.values()) {
                if (change.getNewValue() instanceof WindowNavigationalState) {
                    ...
                    if (wns.getWindowState().equals(WindowState.MAXIMIZED)) {
                        ...
                        //logger.info("MAX KEY " + ((NavigationalStateKey) maximizedKey).getId());
            ...
            if (maximizedKey != null) {
                ...
                for (Object key : keys) {
                    ...
                    if (object instanceof WindowNavigationalState) {
                        ...
                        if (!key.equals(((NavigationalStateKey) maximizedKey).getId().toString()) && wns.getWindowState().equals(WindowState.MAXIMIZED)) {
                            //logger.info("CUR KEY ??? " + key);

                            WindowNavigationalState wns2 = new WindowNavigationalState(WindowState.NORMAL, wns.getMode(), wns.getContentState(),
