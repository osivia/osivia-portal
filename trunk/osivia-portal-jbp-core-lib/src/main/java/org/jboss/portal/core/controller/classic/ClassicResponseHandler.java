...
public class ClassicResponseHandler extends AbstractResponseHandler {
    ...
    public HandlerResponse processCommandResponse(ControllerContext controllerContext, ControllerCommand controllerCommand, ControllerResponse commandResponse)
        ...
        if (handlerResponse instanceof CommandForward) {
            ...
            if (this.requiresRedirect(controllerCommand, urlContext, forward)) {
                String url = controllerContext.renderURL(forward.getCommand(), forward.getURLContext(), URLFormat.newInstance(false, true));
    ...
    private HandlerResponse processHandlers(ControllerContext controllerContext, ControllerCommand command, ControllerResponse commandResponse)
            throws ResponseHandlerException {
    ...
    // Unhardcode this
    private ResponseHandler[] handlers = new ResponseHandler[]{new PortletInstanceResponseHandler(), new PortalObjectResponseHandler(),
            new PortletResponseHandler(), new OtherResponseHandler()};
