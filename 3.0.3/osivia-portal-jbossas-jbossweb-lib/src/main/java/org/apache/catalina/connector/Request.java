...
/*      */
/*  178 */   private final HashMap readOnlyAttributes = new HashMap();
/*      */
...
/*      */
/*  337 */   private final StringParser parser = new StringParser();
/*      */
...
/*      */   public Host getHost()
/*      */   {
/*  547 */     if (this.getContext() == null) {
    /*  548 */       return null;
}
/*  549 */     return (Host)this.getContext().getParent();
/*      */   }
/*      */
...
/*  882 */       return this.requestDispatcherPath == null ? this.getRequestPathMB().toString() : this.requestDispatcherPath.toString();
/*      */     }
/*      */
...
/*      */   {
/*  935 */     if (this.isSecure()) {
/*  936 */       this.getAttribute("javax.servlet.request.X509Certificate");
/*      */     }
...
/*      */     }
/* 1062 */     Enumeration enumeration = this.getParameterNames();
/* 1063 */     while (enumeration.hasMoreElements()) {
/* 1064 */       String name = enumeration.nextElement().toString();
/* 1065 */       String[] values = this.getParameterValues(name);
...
/* 1183 */         this.remoteHost = this.getRemoteAddr();
/*      */       } else {
...
/*      */     }
/*      */
/* 1264 */     String servletPath = (String)this.getAttribute("javax.servlet.include.servlet_path");
/* 1265 */     if (servletPath == null) {
/* 1266 */       servletPath = this.getServletPath();
/*      */     }
/*      */
/* 1269 */     String pathInfo = this.getPathInfo();
...
/* 1357 */     ServletRequestAttributeEvent event = new ServletRequestAttributeEvent(this.context.getServletContext(), this.getRequest(), name, value);
/*      */
...
/*      */
/* 1390 */     if (value == null) {
/* 1391 */       this.removeAttribute(name);
/* 1392 */       return;
/*      */     }
/*      */
...
/* 1428 */     if (replaced) {
/* 1429 */       event = new ServletRequestAttributeEvent(this.context.getServletContext(), this.getRequest(), name, oldValue);
/*      */     }
/*      */     else
/*      */     {
/* 1433 */       event = new ServletRequestAttributeEvent(this.context.getServletContext(), this.getRequest(), name, value);
/*      */     }
/*      */
...
/* 1751 */       HttpSession session = this.getSession(false);
...
/*      */   {
/* 1833 */     String value = this.getHeader(name);
/* 1834 */     if (value == null) {
/* 1835 */       return -1L;
/*      */     }
/*      */
...
/*      */   {
/* 1888 */     String value = this.getHeader(name);
/* 1889 */     if (value == null) {
/* 1890 */       return -1;
/*      */     }
...
/* 1931 */       return null;
/*      */     }
/* 1933 */     if (this.getPathInfo() == null) {
/* 1934 */       return null;
/*      */     }
/* 1936 */     return this.context.getServletContext().getRealPath(this.getPathInfo());
/*      */   }
/*      */
...
/* 2015 */     String scheme = this.getScheme();
/* 2016 */     int port = this.getServerPort();
/* 2017 */     if (port < 0) {
/* 2018 */       port = 80;
/*      */     }
/* 2020 */     url.append(scheme);
/* 2021 */     url.append("://");
/* 2022 */     url.append(this.getServerName());
/* 2023 */     if (((scheme.equals("http")) && (port != 80)) || ((scheme.equals("https")) && (port != 443)))
/*      */     {
/* 2025 */       url.append(':');
/* 2026 */       url.append(port);
/*      */     }
/* 2028 */     url.append(this.getRequestURI());
/*      */
/* 2030 */     return url;
/*      */   }
/*      */
...
/*      */   {
/* 2059 */     Session session = this.doGetSession(true);
...
/*      */   {
/* 2075 */     Session session = this.doGetSession(create);
...
/*      */   public boolean isRequestedSessionIdFromUrl()
/*      */   {
/* 2120 */     return this.isRequestedSessionIdFromURL();
/*      */   }
/*      */
...
/*      */   {
/* 2209 */     return this.doGetSession(true);
/*      */   }
/*      */
/*      */   public Session getSessionInternal(boolean create)
/*      */   {
/* 2220 */     return this.doGetSession(create);
/*      */   }
/*      */
...
/*      */     }
/*      */
/* 2308 */     if ((this.connector.getEmptySessionPath()) && (this.isRequestedSessionIdFromCookie()))
/*      */     {
/* 2310 */       this.session = manager.createSession(this.getRequestedSessionId());
/*      */     } else {
    this.session = manager.createSession(null);
}
/*      */
/* 2316 */     if ((this.session != null) && (this.getContext() != null) && (this.getContext().getCookies()))
/*      */     {
// AJOUT PIA
...
/*      */
/* 2320 */       this.configureSessionCookie(cookie);
...
/* 2341 */     if ((!this.connector.getEmptySessionPath()) && (this.getContext() != null)) {
/* 2342 */       contextPath = this.getContext().getEncodedPath();
/*      */     }
...
/*      */
/* 2404 */     String enc = this.getCharacterEncoding();
/*      */
...
/* 2424 */       return;
/*      */     }
/* 2426 */     if (!this.getMethod().equalsIgnoreCase("POST")) {
/* 2427 */       return;
/*      */     }
/* 2429 */     String contentType = this.getContentType();
...
/* 2439 */       return;
/*      */     }
/* 2441 */     int len = this.getContentLength();
/*      */
/* 2443 */     if (len > 0) {
...
/*      */       }
/*      */       try {
/* 2460 */         if (this.readPostBody(formData, len) != len) {
    /* 2461 */           return;
    /*      */
}       }
/*      */       catch (IOException e)
...
/* 2482 */     int offset = 0;
/*      */     do {
/* 2484 */       int inputLen = this.getStream().read(body, offset, len - offset);
/* 2485 */       if (inputLen <= 0) {
/* 2486 */         return offset;
/*      */       }
...
/*      */
/* 2502 */     Enumeration values = this.getHeaders("accept-language");
/*      */
/* 2504 */     while (values.hasMoreElements()) {
/* 2505 */       String value = values.nextElement().toString();
/* 2506 */       this.parseLocalesHeader(value);
/*      */     }
/*      */   }
/*      */
